...
export default function OrderSummary({ orderId }) {
    ...
    const { orderDetail, orderDetailLoading, invoiceLoading, selectedOrder } = useSelector(({ order }) => order);
                ...
                <div className="flex flex-col gap-4">
                    ...
                    <div className="bg-white p-4 lg:p-6 flex flex-col gap-2 text-sm md:text-base">
                        {billingItems.map((item) => (
                            <div className="flex justify-between" key={`billing-${item.label}`}>
                                <h4 className="font-medium">{item.label}</h4>
                                <p className="font-semibold">{item.value}</p>
                            </div>
                        ))}
                        <p className="font-medium mt-2">
                            *Sales tax will be applied to shipped to addresses within New York State.
                        </p>
                    </div>
                </div>
