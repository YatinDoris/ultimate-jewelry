...
import { handleInvalidAddressErrorMsg } from "../store/actions/addressActions";
...
const validateAddress = (payload, abortController) => async (dispatch) => {
  try {
    if (payload) {
      dispatch({
        type: actionTypes.VALIDATE_ADDRESS_LOADER,
        validateAddressLoader: true,
      });
    ...
    if (error?.code === "ERR_NETWORK") {
      dispatch(
        handleInvalidAddressErrorMsg({
          setInvalidAddressErrorMsg: error?.message,
        })
      );
    }
    return false;
